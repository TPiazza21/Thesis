...
def delta_func(n): # to be able to toggle how delta is used
    #return 10 ** (-6)
    return n ** (-1.1)

def plots_by_gamma(correlated_data=False):
    # generates relative efficiency for synthetic data (correlated_data is not working yet)
    # at one point, there was work to get correlated data, but this is NOT functional now
    if correlated_data:
    ...
    # do the SSP algorithm on its own (not for each gamma)
    ...
    for i in range(num_n):
        ...
        print(f"n is {n}")
        if correlated_data:
            # this is not actually finished!
            #row_collector = []
            #mean_input = np.zeros(d)
            #cov_input = np.eye(d)
            #cov_input = (np.ones((d,d)) + np.eye(d))/2.0
            #cov_input[0,1] = 0.9
            #cov_input[1,0] = 0.9
            #cov_input[0,2] = -0.9
            #cov_input[2,0] = -0.9
            ...
            #cov_input = symmetric_normal_mat
            all_data = scipy.io.loadmat('data/data_uci/' + "wine" + '/' + "wine" + '.mat')['data']


            X_temp = np.array(all_data[:,:10])
            cov_input = (X_temp.T).dot(X_temp)
            #print("X_temp is this")
            #print(X_temp)
            #print(X_temp.shape)
            mean_input = X_temp.mean(axis=0)
            ...
            X = np.random.multivariate_normal(mean_input, cov_input,n)
                #row_collector.append(x_row)
            #X = np.array(row_collector)
            #print(X)
            #print(X.shape)
            #print((X.T).dot(X)/n)
            #assert(False)
        ...
        X = X/X_sqrt_sum[:,None]
        # beware trying to standardize/normalize y, given how it relates with theta
    ...
    for j in range(num_eps):
        ...
        for i in range(num_n):
            ...
            # constant is 3 - for adaSSP, for SSP, constSSPfull
    ...
    # for plotting zeroes (lambda zero proportion)
    for j in range(num_eps):
        eps = epslist[j]
        ax = axs_raveled[j]
    ...
    fig.legend(handles, labels, loc='upper right')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(f"plots/zero_count_by_gamma_all_eps_{data_description}_synthetic.png")
    plt.close()


def plots_by_gamma_pred_err_synth():
    # prediction error with synthetic data
    ...
    # linear regression is necessary to generate, to create relative efficiency denominator
    non_private_results = np.zeros((num_eps, num_n))
...
if __name__ == "__main__":
    ...
    # generates relative efficiency for synthetic data
    #plots_by_gamma(correlated_data=False)

    # generates prediction error for synthetic data
